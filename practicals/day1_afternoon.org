#+TITLE: Practicals Day 1 Afternoon Session
...
- Outcome :: Cardiovascular disease within 5 years of time 0. The is coded
             as 0 for no, 1 for yes, in the variable cvd_5year
...
2. [@2] What is the prevalence of CVD 5 years after time zero?

#+BEGIN_SRC R  :results output :exports (exercise-with-code)  :session *R* :cache yes  :eval (never-plain-export)
100*mean(train$cvd_5year)
#+END_SRC

3. [@3] Note that the null model ignores the covariates and predicts the same risk to
...
4. [@4] Fit a bench mark model using logistic regression including additive
...
5. [@5] Fit a model which resembles the formula of the Steno 1 risk engine using logistic regression
...
6. [@6] Does the Steno model outperform the benchmark model in the train data? Use 10-fold cross-validation.
...
7. [@7] Does the Steno model outperform the benchmark model in the test data? 
...
8. [@8] Use any method to make a new risk prediction model and compare it
   to the Steno model on the test data. Note that the =riskRegression::Score= function
