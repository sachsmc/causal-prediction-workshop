...
-   **Predictors:** all measured at or before time 0 but no more than 1
    year before time 0
...
1.  Load R-package `riskRegression`

2.  Read the train and the test data into R

3.  What is the prevalence of CVD 5 years after time zero?

4.  Note that the null model ignores the covariates and predicts the same risk to
    everyone. It can be obtained with the `glm` function as follows:

5.  Fit a bench mark model using logistic regression including additive
    effects of sex, age and diabetes duration

6.  Fit a model which resembles the formula of the Steno 1 risk engine using logistic regression

7.  Does the Steno model outperform the benchmark model in the train data? Use 10-fold cross-validation

8.  Does the Steno model outperform the benchmark model in the test data?

9.  Use any method to make a new risk prediction model and compare it
