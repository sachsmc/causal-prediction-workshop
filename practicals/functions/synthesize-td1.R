synthesize_td1 <- function(n, beta_cs, beta_ct, beta_ls, beta_st,
    ...
    lava::distribution(sim_model,~value_LDL) <- lava::normal.lvm(mean=2.34380835266364,sd=2.78907040283751)
    ...
    lava::distribution(sim_model,~time.event.1) <- lava::coxWeibull.lvm(scale=0.000000001,shape=3.1383691898171)
    ...
    lava::regression(sim_model,value_SBP~diabetes_duration+age+sex + Statin ) <- c(0.173711840258398,0.408090766933998,5.26413576607228, 3)
    lava::regression(sim_model,value_LDL~diabetes_duration+age+sex) <- c(-0.00215485947423231,-0.00194439878409113,-0.0247941295129788)

    lava::regression(sim_model, value_LDL_followup ~ value_LDL + Statin) <- c(1.25, beta_sm)
    ...
    lava::regression(sim_model,value_Smoking~log2_eGFR_old+log2_eGFR_young+value_AlbuminuriaMicro+value_AlbuminuriaMacro+value_HBA1C+value_LDL+value_SBP+diabetes_duration+age+sex) <- c(-0.573224437209381,-0.533358383827049,0.417778953216348,0.111962094950751,0.0258095967578417,0.128112260674147,-0.0102291702067243,-0.00575108475194562,0.00529875681026287,-0.130756787272673)
    lava::regression(sim_model,value_Motion~value_Smoking+log2_eGFR_old+log2_eGFR_young+value_AlbuminuriaMicro+value_AlbuminuriaMacro+value_HBA1C+value_LDL+diabetes_duration+age+sex) <- c(-0.31142087785739,-0.134144429182559,-0.0862952685994639,-0.121332306983678,-0.334529806176446,-0.0100110418345244,-0.138145721076664,-0.0080292376964733,-0.00517733988644732,0.070748099701443)
    ...
    lava::regression(sim_model,time.event.1~hidden_Comorbidity + Statin + value_LDL_followup + value_Motion+value_Smoking+log2_eGFR_old+log2_eGFR_young+value_AlbuminuriaMicro+value_AlbuminuriaMacro+value_HBA1C+value_LDL+value_SBP+diabetes_duration+age+sex) <- c(beta_ct,beta_st,beta_mt,-0.17,0.21,-0.14,-0.08,0.058,0.061,0.12,0.20,0.016,-0.01,0.04,0.12)
